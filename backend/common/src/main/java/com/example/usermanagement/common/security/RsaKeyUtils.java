...
public final class RsaKeyUtils {
    ...
    public static PrivateKey parsePrivateKey(String value) {
        byte[] bytes = decodeKey(value, "PRIVATE KEY");
        try {
            return KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(bytes));
        } catch (Exception ex) {
            throw new IllegalArgumentException("Invalid RSA private key. Use PKCS#8 PEM or base64 DER.", ex);
        }
    }

    public static PublicKey parsePublicKey(String value) {
